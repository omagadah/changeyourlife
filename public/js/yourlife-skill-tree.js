...
console.log('[SkillTree] Module loading...');
...
async function loadSkillTree(uid) {
  try {
    ...
    if (snap.exists() && snap.data().skillTree) {
      console.log('[SkillTree] Données chargées depuis Firestore');
      return snap.data().skillTree;
    }
  } catch (e) {
    console.error('[SkillTree] Erreur chargement:', e);
  }
  console.log('[SkillTree] Utilisation données par défaut');
...
async function saveSkillTree(uid, data) {
  try {
    ...
    console.log('[SkillTree] Sauvegardé');
  } catch (e) {
    console.error('[SkillTree] Erreur sauvegarde:', e);
    const status = document.getElementById('save-status');
    if (status) status.textContent = 'Erreur sauvegarde!';
  }
}

// Initialiser Cytoscape UNIQUEMENT quand le DOM est prêt
function initCytoscape(uid, skillTree) {
  console.log('[SkillTree] Initialisation Cytoscape...');
  ...
  function updateStats() {
    ...
    if (statNodes) statNodes.textContent = total;
  ...
  console.log('[SkillTree] Cytoscape initialisé ✓');
}

// ATTENDRE que le DOM soit prêt ET que l'auth soit établie
function initApp() {
  console.log('[SkillTree] initApp called');
  
  // Vérifier que le DOM est prêt
  if (document.readyState === 'loading') {
    console.log('[SkillTree] DOM not ready, waiting...');
    document.addEventListener('DOMContentLoaded', initApp);
    return;
  }

  console.log('[SkillTree] DOM ready, checking auth...');

  // Vérifier l'auth
  onAuthStateChanged(auth, async (user) => {
    if (!user) {
      console.log('[SkillTree] No user, redirecting to login');
      window.location.href = '/login';
      return;
    }

    console.log('[SkillTree] User authenticated:', user.uid);
    const uid = user.uid;
    const skillTree = await loadSkillTree(uid);
    
    // Initialiser Cytoscape
    initCytoscape(uid, skillTree);
  });
}

// Lancer l'initialisation
console.log('[SkillTree] Starting...');
initApp();
}
