// @ts-nocheck
// web/lib/tree/tree-model.ts — Arbre de vie procédural CYL (porté de /public/js/tree-model.js).
// Framework-agnostique : reçoit THREE en paramètre. Réutilisé par <TreeCanvas> (R3F/vanilla)
// côté Next.js ET par l'ancien site. Source de vérité géométrique de l'arbre.
...
/** Construit l'arbre. Renvoie { group, nodes, subNodes, grow, branchGroups, addGrassBlades, animateCosmos }. */
export function buildTree(THREE, model, opts) {
  ...
  for (let i = 0; i < gp.count; i++) gp.setZ(i, gp.getZ(i) + (rnd() - 0.5) * 0.7);
  ...
  const bladeMat = new THREE.MeshStandardMaterial({ roughness: 0.85, side: THREE.DoubleSide, flatShading: true });
  const BASE_BLADES = 900;
  const BLADE_COUNT = BASE_BLADES + extraGrass;
  const grassMesh = new THREE.InstancedMesh(bladeGeo, bladeMat, BLADE_COUNT);
  const _bm = new THREE.Matrix4(), _bq = new THREE.Quaternion(), _bp = new THREE.Vector3();
  const _bs = new THREE.Vector3(), _be = new THREE.Euler(), _bc = new THREE.Color();
  ...
  for (let i = 0; i < BLADE_COUNT; i++) {
    ...
    _be.set((rnd() - 0.5) * 0.28, rnd() * Math.PI * 2, (rnd() - 0.5) * 0.2);
    _bq.setFromEuler(_be);
    const sc = 0.55 + rnd() * 1.1;
  ...
  function addGrassBlades(n) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      _e2.set((Math.random() - 0.5) * 0.28, Math.random() * Math.PI * 2, (Math.random() - 0.5) * 0.2);
  ...
  // Cosmos : étoiles + soleil + planètes
  ...
  root.add(new THREE.Points(starGeo, new THREE.PointsMaterial({
  ...
  root.add(sunGroup);
  sunGroup.add(new THREE.Mesh(new THREE.SphereGeometry(45, 32, 20), new THREE.MeshBasicMaterial({ color: 0xffd86a })));
  sunGroup.add(new THREE.Mesh(new THREE.SphereGeometry(85, 32, 20),
  ...
  sunGroup.add(new THREE.Mesh(new THREE.SphereGeometry(150, 32, 20),
  ...
  const planetsData = [
    { color: 0x9a8a7a, radius: 4, dist: 180, speed: 0.18, tilt: 0.04 },
    { color: 0xddc06a, radius: 6, dist: 280, speed: 0.10, tilt: -0.03 },
    { color: 0xd17a48, radius: 5, dist: 380, speed: 0.07, tilt: 0.04 },
    { color: 0xd4a574, radius: 16, dist: 540, speed: 0.04, tilt: 0.02 },
    { color: 0xc9b078, radius: 13, dist: 720, speed: 0.025, tilt: 0.05, ring: true },
    { color: 0x8fc6d4, radius: 8, dist: 920, speed: 0.018, tilt: -0.04 },
    { color: 0x4a78d4, radius: 8, dist: 1120, speed: 0.014, tilt: 0.02 },
  ];
  const planets = [];
  for (const p of planetsData) {
    const mesh = new THREE.Mesh(new THREE.SphereGeometry(p.radius, 20, 14),
    ...
    sunGroup.add(mesh);
    if (p.ring) {
      const ring = new THREE.Mesh(new THREE.RingGeometry(p.radius * 1.5, p.radius * 2.4, 48),
  ...
  function animateCosmos(dt) {
    ...
    for (const p of planets) {
      ...
      p.position.set(Math.cos(a) * p.userData.dist, p.userData.tilt * p.userData.dist, Math.sin(a) * p.userData.dist);
      p.rotation.y += dt * 0.3;
    }
  }
  ...
  for (const b of model.branches) {
    const attachLocal = trunkCurve.getPoint(b.attach);
    ...
    if (b.state === 'active') {
      ...
      const puffCount = 3 + Math.round((b.vitality / 100) * 3);
      const palette = [folMatA, folMatB, folMatC];
      for (let p = 0; p < puffCount; p++) {
        const pr = 2.4 + rnd() * 1.6;
        const fol = new THREE.Mesh(new THREE.IcosahedronGeometry(pr, 1), palette[(rnd() * palette.length) | 0]);
        fol.position.set((rnd() - 0.5) * 3.8, 0.4 + (rnd() - 0.3) * 2.4, (rnd() - 0.5) * 3.8);
        folGroup.add(fol);
      }
    }
    ...
    for (let k = 0; k < grown; k++) {
      ...
      if (b.state === 'active') {
        ...
        twGroup.add(new THREE.Mesh(taperedTube(THREE, twCurve, r1 * 0.45, r1 * 0.16, 8, 5), barkOf(b.state)));
        ...
        if (b.vitality > 8) {
          const twTipWorld = attachLocal.clone().add(sOriginLocal).add(twOriginLocal).add(twTipLocal);
